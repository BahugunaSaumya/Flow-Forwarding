...
public class Router5 extends Node {
/*	public static  String R1 = "Rone";
	...
	*/
	...
	Router5() throws SocketException, UnknownHostException {
		this.routerNumber = PORT_NUMBER ;
		socket = new DatagramSocket(addr.get(R5));
		System.out.println("listening on "+addr.get(R5)+ "container="+values.get(address.get(addr.get(R5))));
	...
	public synchronized void onReceipt(DatagramPacket packet)  {
		...
		try {	handleControllerPacket(packet);}catch(UnknownHostException e) {e.printStackTrace();}
			...
			try {	handleEndNodePacket(packet);}catch(UnknownHostException e) {e.printStackTrace();}	
		}
	}
		...
		case FLOW_MOD:
			...
			for (int j = 0; j < flowTable.length; j++) {
				for (int k = 0; k < flowTable[0].length; k++) {
					System.out.print("" + flowTable[j][k] + " ");
				}
				System.out.println("");
			}
	...
	*/
	...
	private synchronized void setEndNodeAddress() throws UnknownHostException {
		boolean addressSet = false;
		int i = 0;
		while (i < flowTable.length && !addressSet) {
			
			if (flowTable[i][Router_INDEX]==R5 &&flowTable[i][INPUT_INDEX] > NUM_SWITCHES) {
			...
			} else if (flowTable[i][Router_INDEX]==R5 &&flowTable[i][OUTPUT_INDEX] > NUM_SWITCHES) {
	...
	private synchronized void handleEndNodePacket(DatagramPacket packet) throws UnknownHostException {
		byte[] data = packet.getData();
	...
	private synchronized byte checkFlowtable(byte[] data, InetAddress port) {
		...
		/*	System.out.println("flo "+flowTable[i][SRC_INDEX]);
			...
			if (src==flowTable[i][SRC_INDEX] && dst==flowTable[i][DST_INDEX] && prev==flowTable[i][INPUT_INDEX]) {
				...
				System.out.println("fldsfs5 " +nextHop);
			}
			i++;
		}
		return (byte)nextHop;
