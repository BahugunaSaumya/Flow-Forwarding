...
/*	public static  String R1 = "Rone";
	...
	*/
	...
	Router3() throws SocketException, UnknownHostException {
		this.routerNumber = PORT_NUMBER ;
		socket = new DatagramSocket(addr.get(R3));
		System.out.println("listening on "+addr.get(R3)+ "container="+values.get(address.get(addr.get(R3))));
	...
	*/
	...
	private synchronized void setEndNodeAddress() throws UnknownHostException {
		...
		while (i < flowTable.length && !addressSet) {
			...
			if (flowTable[i][Router_INDEX]==R3 &&flowTable[i][INPUT_INDEX] > NUM_SWITCHES) {
			...
			} else if (flowTable[i][Router_INDEX]==R3 &&flowTable[i][OUTPUT_INDEX] > NUM_SWITCHES) {
