...
/*	public static  String R1 = "Rone";
	...
	*/
	...
	Router2() throws SocketException, UnknownHostException {
		this.routerNumber = PORT_NUMBER ;
		socket = new DatagramSocket(addr.get(R2));
		System.out.println("listening on "+addr.get(R2)+ "container="+values.get(address.get(addr.get(R2))));
	...
	*/
	...
	private synchronized void setEndNodeAddress() throws UnknownHostException {
		...
		while (i < flowTable.length && !addressSet) {
			...
			if (flowTable[i][Router_INDEX]==R2 &&flowTable[i][INPUT_INDEX] > NUM_SWITCHES) {
			...
			} else if (flowTable[i][Router_INDEX]==R2 &&flowTable[i][OUTPUT_INDEX] > NUM_SWITCHES) {
