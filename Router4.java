...
/*	public static  String R1 = "Rone";
	...
	*/
	...
	Router4() throws SocketException, UnknownHostException {
		this.routerNumber = PORT_NUMBER ;
		socket = new DatagramSocket(addr.get(R4));
		System.out.println("listening on "+addr.get(R4)+ "container="+values.get(address.get(addr.get(R4))));
	...
	*/
	...
	private synchronized void setEndNodeAddress() throws UnknownHostException {
		...
		while (i < flowTable.length && !addressSet) {
			...
			if (flowTable[i][Router_INDEX]==R4 &&flowTable[i][INPUT_INDEX] > NUM_SWITCHES) {
			...
			} else if (flowTable[i][Router_INDEX]==R4 &&flowTable[i][OUTPUT_INDEX] > NUM_SWITCHES) {
