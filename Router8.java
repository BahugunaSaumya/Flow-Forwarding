...
public class Router8 extends Node {
/*	public static  String R1 = "Rone";
	...
	*/
	...
	Router8() throws SocketException, UnknownHostException {
		this.routerNumber = PORT_NUMBER ;
		socket = new DatagramSocket(addr.get(R8));
		System.out.println("listening on "+addr.get(R8)+ "container="+values.get(address.get(addr.get(R8))));
	...
	*/
	...
	private synchronized void setEndNodeAddress() throws UnknownHostException {
		...
		while (i < flowTable.length && !addressSet) {
			...
			if (flowTable[i][Router_INDEX]==R8 &&flowTable[i][INPUT_INDEX] > NUM_SWITCHES) {
			...
			} else if (flowTable[i][Router_INDEX]==R8 &&flowTable[i][OUTPUT_INDEX] > NUM_SWITCHES) {
