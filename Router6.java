...
public class Router6 extends Node {
/*	public static  String R1 = "Rone";
	...
	*/
	...
	Router6() throws SocketException, UnknownHostException {
		this.routerNumber = PORT_NUMBER ;
		socket = new DatagramSocket(addr.get(R6));
		System.out.println("listening on "+addr.get(R6)+ "container="+values.get(address.get(addr.get(R6))));
	...
	*/
	...
	private synchronized void setEndNodeAddress() throws UnknownHostException {
		...
		while (i < flowTable.length && !addressSet) {
			...
			if (flowTable[i][Router_INDEX]==R6 &&flowTable[i][INPUT_INDEX] > NUM_SWITCHES) {
			...
			} else if (flowTable[i][Router_INDEX]==R6 &&flowTable[i][OUTPUT_INDEX] > NUM_SWITCHES) {
