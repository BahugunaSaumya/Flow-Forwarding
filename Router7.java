...
public class Router7 extends Node {
/*	public static  String R1 = "Rone";
	...
	*/
	...
	Router7() throws SocketException, UnknownHostException {
		this.routerNumber = PORT_NUMBER ;
		socket = new DatagramSocket(addr.get(R7));
		System.out.println("listening on "+addr.get(R7)+ "container="+values.get(address.get(addr.get(R7))));
	...
	*/
	...
	private synchronized void setEndNodeAddress() throws UnknownHostException {
		...
		while (i < flowTable.length && !addressSet) {
			...
			if (flowTable[i][Router_INDEX]==R7 &&flowTable[i][INPUT_INDEX] > NUM_SWITCHES) {
			...
			} else if (flowTable[i][Router_INDEX]==R7 &&flowTable[i][OUTPUT_INDEX] > NUM_SWITCHES) {
